...
- name: add vlan
  ...
  - ax_vid: '2000'
  - ax_ipaddr: '192.168.0.1'
  - ax_netmask: '255.255.255.0'
  - ax_portif : '0/2'
  ...
  - name: configure interface vlan <vid>
    ax_config:
      lines:
      - 'ip address {{ ax_ipaddr }} {{ ax_netmask }}'
      parents: 'interface vlan {{ ax_vid }}'
  
  - name: vlan configuration to port
    ax_config:
      lines:
      - 'switchport mode trunk'
      - 'switchport trunk allowed vlan add {{ ax_vid }}'
      parents: 'interface gigabitethernet {{ ax_portif }}'
      save_when: modified
```
■Playbook内容の説明
...
VLAN番号はvarsで定義した変数(ax_vid)を参照しています。  
<br>
...
ipaddrとnetmaskはvarsで定義した変数(ax_ipaddr, ax_netmask)を参照しています。  

- parents: "interface vlan {{ ax_vid }}"  
linesオプションで設定したコンフィグレーションリストを設定する階層のリストを設定します。  
IPアドレスとサブネットマスクを設定する階層として、interface vlan {{ ax_vid }}を設定しています。  
VLAN番号はvarsで定義した変数(ax_vid)を参照しています。  
<br>
...
- parents: "interface gigabitethernet {{ ax_portif }}"  
トランクモードを設定する階層として、interface gigabitethernet {{ ax_portif }}を指定しています。  
ポート番号はvarsで定義した変数(ax_portif)参照しています。  

- lines:  
switchport trunk allowed vlan add {{ ax_vid }}  
トランクポートにVLANを追加しています。  
追加するVLANのVLAN番号はvarsで定義した変数(ax_vid)を参照しています。  
